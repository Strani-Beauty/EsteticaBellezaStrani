-- =============================================================================
-- Migración: elimina el trigger `tr_log_cita_estado` (historial CITA con una
-- sola fuente).
-- -----------------------------------------------------------------------------
-- Opción 1 acordada: el trigger `tr_log_cita_estado` (creado fuera del repo en
-- el SQL Editor) duplicaba el historial CITA porque la app (`treatment_execution`)
-- ya registra las transiciones manualmente en `historial_estados`. Se elimina el
-- trigger y su función, y se restaura en `aceptar_solicitud` el registro de la
-- cita creada (PROGRAMADA) — antes lo cubría el trigger; sin él, la creación
-- quedaría sin historial. Fuente única tras esto:
--   * Creación de cita  → `aceptar_solicitud` (security definer).
--   * Transiciones      → app (`treatment_execution._insertHistorial`).
-- Idempotente: DROP ... IF EXISTS + CREATE OR REPLACE.
-- =============================================================================

DROP TRIGGER IF EXISTS tr_log_cita_estado ON public.citas;
DROP FUNCTION IF EXISTS public.log_cita_estado_change();
